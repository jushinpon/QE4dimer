...
my $dir = "$currentPath/dimer_data";#source dir for data files
#my $dir = "$currentPath/2dimers_data";#source dir for data files
...
my $temperature = 300.00;#temperatures for QE_MD
my $pressure = 0.0;#pressure for vc-md, 
my $calculation = "scf";#set temperature and pressure to be 0 0 for scf
#my $calculation = "md";#set temperature and pressure to be 0 0 for scf


my $stepsize = 50;#20 ~ 0.97 fs
my $nstep = 5;#how many steps for md for vc-relax
...
for my $id (@datafile){
    ...
    my %myelement;#get all elements from each data file
    ...
    #my @ele = `grep -v '^[[:space:]]*\$' $_|grep -A $typeNum Masses|grep -v Masses|grep -v -- '--'|awk '{print \$NF}'`;
    map { s/^\s+|\s+$//g; } @ele;
    die "No Masses for finding element symbol in $id\n" unless(@ele);
    @myelement{@ele} = 1;
...
#1 1 1.000000 4.458517505863 1.201338326940 0.873835074284 with charge
    ...
    my %QE_para = (
            ...
            output_file => "$store_path/$data_name/$QEoutput",
            output_dir => "$store_path/$data_name",
            pseudo_dir => $pseudo_dir,
            coord => $coords,
            temp => $temperature, #temperature for md
            press => $pressure, #pressure for vc-md
            ntyp => $current_typeNu,
